...
import { WebviewOriginStore, IWebviewService, WebviewContentPurpose } from '../../../../webview/browser/webview.js';
...
import { ToolMcpUiOutput } from '../../../common/languageModelToolsService.js';
...
export class ChatMcpAppSubPart extends BaseChatToolInvocationSubPart {
	...
	private static readonly _cachedStates = new WeakMap<IChatViewModel | IChatToolInvocationSerialized, Map<string, McpAppState>>();
	...
	private _createWebviewContainer(): HTMLElement {
		const vm = this._chatWidgetService.getWidgetBySessionResource(this._context.element.sessionResource)?.viewModel;
		...
		if (vm) {
			let allStates = ChatMcpAppSubPart._cachedStates.get(vm);
			if (!allStates) {
				allStates = new Map<string, McpAppState>();
				ChatMcpAppSubPart._cachedStates.set(vm, allStates);
			}
	...
	private async _loadAndRenderApp(
		...
		this._register(webview.onMessage(async ({ message }) => {
			if (message.method === 'ui/notifications/size-changed') {
				const params = message.params as McpApps.McpUiSizeChangedNotification['params'];
				if (params && typeof params.height === 'number' && params.height !== state.height) {
			...
			await this._handleWebviewMessage(message as McpApps.AppRequest, webview);
		}));
	 ...
	 */
	private async _handleWebviewMessage(message: McpApps.AppRequest, webview: { postMessage(message: unknown): Promise<boolean> }): Promise<void> {
	...
	private async _sendResponse(webview: { postMessage(message: unknown): Promise<boolean> }, id: number | string, result: any): Promise<void> {
	...
	private async _sendError(webview: { postMessage(message: unknown): Promise<boolean> }, id: number | string, code: number, message: string): Promise<void> {
		await webview.postMessage({
			jsonrpc: '2.0',
			id,
			error: { code, message },
