...
import { findMcpServer } from '../common/mcpTypesUtils.js';
...
export class McpToolCallUI extends Disposable {
	...
	public async readResource(uri: string, token: CancellationToken): Promise<MCP.ReadResourceResult> {
		...
		const handler = server.connection.get()?.handler.get();
		if (!handler) {
			throw new Error('MCP server connection not available');
		}

		this._logService.debug(`[MCP App] Reading resource: ${uri}`);
		return handler.readResource({ uri }, token);
	}
}
